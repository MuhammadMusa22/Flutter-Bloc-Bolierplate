...
class ExampleCubit extends Cubit<ExampleCubitState>{
  ...
  ExampleCubit({required this.exampleRepository}) : super(ExampleCubitInitialState());

  void getExampleValues(bool throwError) async {
    try {
      emit(ExampleCubitLoadingState());
      final List<String>? exampleValues = await exampleRepository.getExampleValues(throwError);
